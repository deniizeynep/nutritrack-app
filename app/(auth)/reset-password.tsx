...
export default function ResetPasswordScreen() {
  ...
  const pendingEmail = useAuthStore((state) => state.pendingPasswordResetEmail);
  const isLoading = useAuthStore((state) => state.isLoading);
  const resetPassword = useAuthStore((state) => state.resetPassword);
  const clearPendingPasswordReset = useAuthStore(
    (state) => state.clearPendingPasswordReset,
  );
  ...
  const handleReset = async () => {
    ...
    if (newPassword.length < 8) {
      Alert.alert(
        translate("resetPassword", language),
        translate("passwordMinError", language),
        [{ text: translate("ok", language) }],
      );
      return;
    }
    ...
    if (!/[A-Za-z]/.test(newPassword) || !/[0-9]/.test(newPassword)) {
      Alert.alert(
        translate("resetPassword", language),
        translate("passwordLetterNumberError", language),
        [{ text: translate("ok", language) }],
      );
      return;
    }
    ...
    if (newPassword !== confirmPassword) {
      Alert.alert(
        translate("resetPassword", language),
        translate("passwordsDoNotMatch", language),
        [{ text: translate("ok", language) }],
      );
      return;
    }
    ...
    try {
      ...
      Alert.alert(
        translate("resetPassword", language),
        translate("passwordResetSuccess", language),
        [{ text: translate("ok", language) }],
      );
        ...
        >
          ...
          >
            ...
            <Input
              ...
              placeholder={translate("newPasswordPlaceholder", language)}
              value={newPassword}
